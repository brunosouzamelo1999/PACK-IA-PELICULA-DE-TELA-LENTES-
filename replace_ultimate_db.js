...
const startIdx = html.indexOf('ultimate: {');
const endIdx = html.indexOf('const templates = {');

if (startIdx !== -1 && endIdx !== -1) {
    const before = html.substring(0, startIdx);
    const after = html.substring(endIdx);
    html = before + ultimateBlock + '\n        };\n\n        ' + after;
    fs.writeFileSync(htmlPath, html, 'utf8');
    console.log('Successfully replaced ultimate block in apresentacao_frame.html');
} else {
    console.error('Could not locate startIdx or endIdx', startIdx, endIdx);
}
